                    ...
                    <div class="m_datatable m-datatable m-datatable--brand m-datatable--default m-datatable--loaded">
                        <table class="m-datatable__table" id="datatable" style="display: block; min-height: 300px;">
                            ...
                            <tbody class="m-datatable__body">
                                <span v-if="tags.length == 0" class="m-datatable--error">No records found</span>
                                <tr :class="'m-datatable__row ' + (index % 2 == 1 ? 'm-datatable__row--even' : '')" v-for="(tag, index) in tags" :key="index">
                                    <td class="m-datatable__cell"><span style="width: 150px;">{{ tag.name }}</span></td>
                                    <td data-field="Actions" class="m-datatable__cell">
                                        <span style="overflow: visible; width: 70px;">
                                            <a @click="editTag(tag.id)" href="#" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit details">
                                                <i class="la la-edit"></i>
                                            </a>
                                            <a @click="deleteTag(tag.id)" href="#" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="Delete">
        ...
        <div class="modal fade" id="create_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <form @submit.prevent="createTag()" class="m-form m-form--fit m-form--label-align-right" style="">
                        <div class="modal-body" id="create_frm">
                            <div class="form-group m-form__group row">
                                ...
                                <div class="col-10">
                                    <input v-model="newTag.name" class="form-control m-input" type="text">
                                </div>
                            </div>
                        </div>
        ...
        <div class="modal fade" id="edit_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <form @submit.prevent="saveTag()" class="m-form m-form--fit m-form--label-align-right" style="">
                        <div class="modal-body" id="create_frm">
                            <div class="form-group m-form__group row">
                                <label class="col-2 col-form-label">
                                    Name
                                ...
                                <div class="col-10">
                                    <input v-model="currentTag.name" class="form-control m-input" type="text">
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
    ...
    export default {
        ...
        methods: {
            ...
            async createTag() {
                ...
                await KbApi.createTag(
                    this.newTag,
                    (data) => {
                        this.notification("Tag created", "success");
            ...
            async deleteTag(id) {
                if (confirm("Are you sure you want to delete this tag?")) {
                    await KbApi.deleteTag(
                        id,
                        (data) => {
                            this.notification("Tag deleted", "success");
                            this.loadPage(this.currentPage);
                        },
                        (err) => this.notification(err, "danger")
                    );
